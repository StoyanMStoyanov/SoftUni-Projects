...
class SlicingFile
{
    static void Main()
    {
        ...
        List<string> gzFilesInDirectory = Directory.GetFiles(destinationFilePath, "*.*").Select(path => Path.GetFileName(path)).ToList<string>();
        Assemble(gzFilesInDirectory, destinationFilePath);
    }
    ...
    private static void Assemble(List<string> files, string destinationFilePath)
    {
        ...
        using (var destination = new FileStream(pathName, FileMode.Create))
        {
            foreach (var item in files)
            {
                ...
                using (var tempSource = new FileStream(destinationFilePath + item, FileMode.Open))
                {
                    ...
                    using (var compressionStream = new GZipStream(tempSource, CompressionMode.Decompress,  false))
                    {
                        int readFile = compressionStream.Read(readFromFile, 0, readFromFile.Length);
                        while (readFile != 0)
                        {
                            destination.Write(readFromFile, 0, readFile);
                            readFile = compressionStream.Read(readFromFile, 0, readFile);
                        }
                    }
                }
            }
    ...
    private static void Slice(string sourceFile, string destinationDirectory, int parts)
    {
        ...
        DirectoryInfo twoLevelsUp = currentDir.Parent.Parent;
        Console.WriteLine("Slice and compress file: {0}.\nIn directory: {1}.", sourceFile.Split('/')[2], twoLevelsUp.FullName);
    }
}